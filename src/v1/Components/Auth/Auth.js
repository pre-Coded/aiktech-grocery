...
const Auth = (props) => {
  ...
  useEffect(() => {
    if (location.coordinates) {
      const inventory = getNearestInventory(
        inventoryList,
  ...
  const isValidPassword = (value) => {
    ...
    return value.length >= minLength && containsLetter && containsNumber;
  };
  ...
  const sendOtp = async (message) => {
    ...
    try {
      ...
      const msg = errorMsg(error);
      toast.error("Please register or check the number");
    }
    setOtpLoading(false);
  };
  ...
  const loginHandler = async () => {
    ...
    try {
      ...
      if (!validationError) {
        ...
        setShowErr(true);
        toast.error("Please type valid phone number and password");
      }
    } catch (error) {
      const msg = errorMsg(error);
  ...
  const loginSuccess = (res) => {
    ...
    storeToken(token);
    dispatch(
      actionsCreator.SET_LOGIN({ isLoggedIn: true, showLoginPopup: false })
    );
      ...
      <div className="login-container">
        <div className="login-options">
          <h5
            className={
              login_option === "login"
                ? "login-head highlighted-head"
                : "login-head"
            }
            onClick={() => {
              setlogin_option("login");
            }}
          >
            Login
          </h5>
          <h5
            className={
              login_option === "signup"
                ? "signup-head highlighted-head"
                : "signup-head"
            }
            onClick={() => {
              setlogin_option("signup");
            }}
          >
            Sign-Up
          </h5>
        ...
        <hr
          className={
            login_option === "login" ? "login-left-hr" : "login-right-hr"
          }
        />
